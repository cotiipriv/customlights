...
/** 1.21.11: the loader is reached through its current instance. The loader queries the shared code makes (installed mods, config folder, this mod's version). */
public final class FabricLoader {
    private static final FabricLoader INSTANCE = new FabricLoader();
    /** NeoForge ports of the Fabric mods the shared code looks for (both keep their own id here). */
    private static final Map<String, String> NEOFORGE_NAMES = Map.of("iris", "iris", "sodium", "sodium");
    ...
    public boolean isModLoaded(String id) {
        ...
        var mods = FMLLoader.getCurrent().getLoadingModList();
        if (mods == null) {
            return false;
        }
