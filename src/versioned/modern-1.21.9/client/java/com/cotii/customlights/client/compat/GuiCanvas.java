...
/**
 * 1.21.9 and later (as 1.21.6, without a depth per vertex): the screen is drawn later from what was submitted, so the
 * gizmo's quads are gathered and handed over as one element of the screen (already placed with the current pose).
 */
        ...
        public void buildVertices(VertexConsumer consumer) {
            ...
            for (int quad = 0; quad < count; quad++) {
                ...
                for (int corner : new int[]{0, 1, 2, 3, 3, 2, 1, 0}) {
                    ...
                    consumer.addVertex(point.x, point.y, 0.0f).setColor(color);
                }
            }
        }
