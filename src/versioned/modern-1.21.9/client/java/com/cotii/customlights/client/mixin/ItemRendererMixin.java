...
/**
 * 1.21.9 and later: an item layer is handed over (already placed for its display context) to be drawn later, so its light is
 * placed as it is handed over.
 */
...
public abstract class ItemRendererMixin {
    ...
    private void customlights$markers(net.minecraft.client.renderer.SubmitNodeCollector collector, PoseStack poseStack, ItemDisplayContext context,
                                      int light, int overlay, int outline, int[] tints, List<BakedQuad> model, RenderType renderType,
        ...
        original.call(collector, poseStack, context, light, overlay, outline, tints, model, renderType, foil);
    }
}
