...
public final class ObjectPreview {
    ...
    public static void register() {
        // Handed over while the frame collects what it draws
        LevelRenderEvents.COLLECT_SUBMITS.register(ObjectPreview::render);
    }
    ...
    private static void render(LevelRenderContext context) {
        Minecraft minecraft = Minecraft.getInstance();
        ClientLevel level = minecraft.level;
        if (stage == null || rule == null || level == null || context.submitNodeCollector() == null) {
            return;
        }
        ...
        PoseStack poseStack = context.poseStack() != null ? context.poseStack() : new PoseStack();
        ...
        try {
            if (!stack.isEmpty()) {
                ...
                new BlockModelResolver(minecraft.getModelManager()).update(state, blockState, BlockDisplayContext.create());
                state.submit(poseStack, queue, light, OverlayTexture.NO_OVERLAY, 0);
            } else if (entity != null) {
